...
describe('ubicaciones y descubrimiento', () => {
  it('define ubicaciones y estaciones con identificadores únicos', () => {
    expect(locations).toHaveLength(14);
    expect(new Set(locations.map((location) => location.id)).size).toBe(14);
    expect(
      locations.filter((location) => location.type === 'station'),
    ).toHaveLength(2);
  });
  ...
  it('descubre San Salvador al iniciar dentro de su radio', () => {
    const discovered = findDiscoverableLocations(
      [-89.191111, 13.6975],
      [],
      ['san-salvador'],
    );
  ...
  it('calcula proximidad y descarta lugares fuera del umbral', () => {
    expect(
      distanceBetweenMeters([-89.191111, 13.6975], [-89.191111, 13.6975]),
    ).toBe(0);
    expect(findNearestLocation([-89.191111, 13.6975], 500)?.id).toBe(
      'san-salvador',
    );
